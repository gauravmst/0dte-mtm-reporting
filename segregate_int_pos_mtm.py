...
# toh is code m basically jo niche file daalni padegi compile user mtm jisme saved mtm se updated hoke aayi wo wali file fr 
# date bhi change krni hogi thik line on 127 m fr code chalana h or fr tumse 4dte ki combined max loss file mangega agr nf h toh dedena 
# or nhi h toh enter dba dena fr 1dte ki file mangega uska path dedena or fr file ko kholna or jo algo chalne wale ho unko rkh lena 
# or niche grand total m bs formula laga dena 
...
#   data_hex   : ~10-12% saturation, used for ALL data + sub-total rows in the algo
...
def write_subtotal_row(row, data_start, data_end, fill, n_servers, is_bold=True, bdr=None):
...
def write_total_row(row, label, n_servers, ref_rows, fill, is_bold=True, bdr=None):
...
for i, algo in enumerate(algos):
    ...
    d_fill, a_fill = _algo_fills(i)   # data fill + accent fill for this algo
    ...
    for ttype, display_label in [("Intraday", "Int"), ("Positional", "Pos+Int")]:
        ...
        data_end = r - 1
        sub_row  = r
        # sub-total uses same algo data fill; bold + medium-bottom border sets it apart
        write_subtotal_row(r, block_start, data_end, d_fill, n_servers=len(rows))
        ws.row_dimensions[r].height = 18
        r += 1
...
# ====================== GRAND TOTAL ======================
write_total_row(r, "Grand Total", total_n_servers, algo_total_rows, grand_fill, bdr=grand_bdr)
...
widths = [7, 9, 12, 18, 13, 13, 14, 15, 13, 10]
